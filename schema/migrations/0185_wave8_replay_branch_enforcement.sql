-- Migration: 0185_wave8_replay_branch_enforcement.sql
-- Task: TSK-P2-W8-DB-007c
-- Purpose: Restate replay enforcement as DB-007c-owned closure evidence
-- Dependencies: TSK-P2-W8-DB-007c
-- Type: Forward-only migration

-- Update the cryptographic enforcement function to add replay prevention enforcement
-- This isolates replay enforcement into DB-007c-owned closure evidence
...
-- Update comment to reflect DB-007c ownership
COMMENT ON FUNCTION public.wave8_cryptographic_enforcement() IS
    'Wave 8 cryptographic enforcement function - updated in 0185 to add DB-007c replay prevention enforcement. Enforces nonce uniqueness via wave8_attestation_nonces table to prevent replay attacks. Hard-fails until SEC-002 provides PostgreSQL native Ed25519 primitive.';

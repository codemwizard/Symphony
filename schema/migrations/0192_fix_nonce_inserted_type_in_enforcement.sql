-- Migration: 0192_fix_nonce_inserted_type_in_enforcement.sql
-- Task: Devin Review remediation
-- Purpose: Fix nonce_inserted variable type from boolean to integer
-- Dependencies: 0190_wave8_cryptographic_enforcement_full_restore.sql
-- Type: Forward-only migration
--
-- Bug: In migration 0190, nonce_inserted is declared as boolean (line 43) but
-- GET DIAGNOSTICS nonce_inserted = ROW_COUNT (line 162) returns an integer.
-- PostgreSQL has no implicit cast from integer to boolean, causing a runtime
-- type error on every INSERT to asset_batches that reaches replay prevention.
-- The comparison IF nonce_inserted = 0 (line 164) also fails (boolean vs integer).
--
-- The same bug exists in 0181, 0185, and 0186 but those functions are superseded
-- by 0190's CREATE OR REPLACE, so only 0190 needs correction.
--
-- Fix: Redeclare nonce_inserted as integer and update the enforcement function.
...
COMMENT ON FUNCTION public.wave8_cryptographic_enforcement() IS
    'Wave 8 cryptographic enforcement function - fixed in 0192 to correct nonce_inserted type from boolean to integer. Validates signature presence, resolves signer from authoritative surface, enforces key lifecycle (P7813), scope authorization (P7810), timestamp integrity (P7811), replay prevention (P7812), mandatory context binding (P7814), and Ed25519 signature verification (P7809) via wave8_crypto extension.';

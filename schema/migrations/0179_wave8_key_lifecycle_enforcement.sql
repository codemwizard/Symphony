-- Migration: 0179_wave8_key_lifecycle_enforcement.sql
-- Task: TSK-P2-W8-DB-008
-- Purpose: Enforce key lifecycle state in the authoritative verification path
-- Dependencies: TSK-P2-W8-DB-006, TSK-P2-W8-DB-007a, TSK-P2-W8-DB-007b, TSK-P2-W8-DB-007c
-- Type: Forward-only migration

-- Add key lifecycle fields to wave8_signer_resolution table
...
BEGIN
    ...
    -- Explicit superseded-key behavior: superseded keys are rejected
...
COMMENT ON COLUMN public.wave8_signer_resolution.superseded_by IS
    'Wave 8 key lifecycle field: References the signer_id that superseded this key. If non-null, this key is superseded and cannot sign.';

COMMENT ON COLUMN public.wave8_signer_resolution.superseded_at IS
    'Wave 8 key lifecycle field: Timestamp when this key was superseded.';

COMMENT ON FUNCTION public.wave8_cryptographic_enforcement() IS
    'Wave 8 cryptographic enforcement function - integrates Ed25519 verification primitive into the authoritative PostgreSQL write path. Validates signature presence, resolves signer from authoritative surface, enforces key lifecycle (revoked, expired, superseded) with P7813 failure mode, enforces scope authorization (P7810), timestamp integrity (P7811), replay prevention (P7812), and fail-closed rejection for invalid signatures.';

...
BEGIN
    ...
    VALUES (gen_random_uuid(), v_proj, 'TEST_ENTITY', gen_random_uuid(), 'A', 'B', NOW(), v_exec, v_pol, 'testhash5', repeat('0', 128));
END $$;
ROLLBACK;
EOF
if [ $? -ne 0 ]; then
...
EOF
then
    echo "ERROR: Behavioral negative test failed (database accepted transition without signature)."
    exit 1
else

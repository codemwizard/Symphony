-- Verification SQL for Wave 8 persisted timestamp enforcement
-- Task: TSK-P2-W8-DB-007b
-- Purpose: Prove that PostgreSQL distinguishes regenerated-timestamp failures at asset_batches

-- Check 1: Verify cryptographic enforcement function exists with timestamp integrity
SELECT 
    p.proname as function_name,
    p.prosecdef as security_definer
FROM pg_proc p
JOIN pg_namespace n ON p.pronamespace = n.oid
WHERE n.nspname = 'public'
AND p.proname = 'wave8_cryptographic_enforcement';

-- Expected: function exists with prosecdef = true

-- Check 2: Verify function includes timestamp integrity logic
DO $$
DECLARE
    function_source text;
BEGIN
    SELECT prosrc INTO function_source
    FROM pg_proc p
    JOIN pg_namespace n ON p.pronamespace = n.oid
    WHERE n.nspname = 'public'
    AND p.proname = 'wave8_cryptographic_enforcement';
    
    IF function_source LIKE '%P7811%' AND function_source LIKE '%timestamp integrity%' THEN
        RAISE NOTICE 'Timestamp integrity logic present: SUCCESS';
    ELSE
        RAISE NOTICE 'Timestamp integrity logic present: FAILURE';
    END IF;
END $$;

-- Expected: Timestamp integrity logic present: SUCCESS

-- Check 3: Test missing canonical payload bytes rejection (timestamp integrity failure)
DO $$
DECLARE
    test_result text;
    test_signer_id uuid;
    test_project_id uuid;
BEGIN
    -- Insert test signer with correct scope
    test_project_id := gen_random_uuid();
    INSERT INTO public.wave8_signer_resolution (
        key_id,
        key_version,
        public_key_bytes,
        project_id,
        scope,
        is_active
    ) VALUES (
        'test_key_007b',
        '1',
        decode('deadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeef', 'hex'),
        test_project_id,
        'wave8_global',
        true
    ) RETURNING signer_id INTO test_signer_id;
    
    -- Attempt to insert without canonical payload bytes (timestamp integrity failure)
    BEGIN
        INSERT INTO public.asset_batches (
            asset_batch_id,
            tenant_id,
            project_id,
            batch_type,
            quantity,
            status,
            created_at,
            data_authority,
            audit_grade,
            authority_explanation,
            transition_hash,
            signature_bytes,
            signer_key_id,
            signer_key_version
        ) VALUES (
            gen_random_uuid(),
            gen_random_uuid(),
            test_project_id,
            'test',
            1,
            'PENDING',
            now(),
            'test_authority'::data_authority_level,
            false,
            'Missing canonical payload test',
            'c9a3b4c5d6e7f8a9b0c1d2e3f4a5b6c7d8e9f0a1b2c3d4e5f6a7b8c9d0e1f2a3',
            decode('deadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeef', 'hex'),
            'test_key_007b',
            '1'
        );
        test_result := 'MISSING_CANONICAL_PAYLOAD_ACCEPTED';
    EXCEPTION WHEN OTHERS THEN
        test_result := 'MISSING_CANONICAL_PAYLOAD_REJECTED';
    END;
    
    RAISE NOTICE 'Missing canonical payload rejection test result: %', test_result;
    
    -- Clean up
    DELETE FROM public.wave8_signer_resolution WHERE signer_id = test_signer_id;
END $$;

-- Expected: MISSING_CANONICAL_PAYLOAD_REJECTED

-- Check 4: Test valid canonical payload acceptance (timestamp integrity pass)
DO $$
DECLARE
    test_result text;
    test_signer_id uuid;
    test_project_id uuid;
    test_asset_batch_id uuid;
BEGIN
    -- Insert test signer with correct scope
    test_project_id := gen_random_uuid();
    INSERT INTO public.wave8_signer_resolution (
        key_id,
        key_version,
        public_key_bytes,
        project_id,
        scope,
        is_active
    ) VALUES (
        'test_key_007b_valid',
        '1',
        decode('deadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeef', 'hex'),
        test_project_id,
        'wave8_global',
        true
    ) RETURNING signer_id INTO test_signer_id;
    
    -- Attempt to insert with valid canonical payload bytes
    test_asset_batch_id := gen_random_uuid();
    BEGIN
        INSERT INTO public.asset_batches (
            asset_batch_id,
            tenant_id,
            project_id,
            batch_type,
            quantity,
            status,
            created_at,
            data_authority,
            audit_grade,
            authority_explanation,
            canonical_payload_bytes,
            transition_hash,
            signature_bytes,
            signer_key_id,
            signer_key_version
        ) VALUES (
            test_asset_batch_id,
            gen_random_uuid(),
            test_project_id,
            'test',
            1,
            'PENDING',
            now(),
            'test_authority'::data_authority_level,
            false,
            'Valid canonical payload test',
            decode('', 'hex'),
            'c9a3b4c5d6e7f8a9b0c1d2e3f4a5b6c7d8e9f0a1b2c3d4e5f6a7b8c9d0e1f2a3',
            decode('deadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeef', 'hex'),
            'test_key_007b_valid',
            '1'
        );
        test_result := 'VALID_CANONICAL_PAYLOAD_ACCEPTED';
        
        -- Clean up test record
        DELETE FROM public.asset_batches WHERE asset_batch_id = test_asset_batch_id;
    EXCEPTION WHEN OTHERS THEN
        test_result := 'VALID_CANONICAL_PAYLOAD_REJECTED';
    END;
    
    RAISE NOTICE 'Valid canonical payload acceptance test result: %', test_result;
    
    -- Clean up
    DELETE FROM public.wave8_signer_resolution WHERE signer_id = test_signer_id;
END $$;

-- Expected: VALID_CANONICAL_PAYLOAD_ACCEPTED

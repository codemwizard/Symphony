...
function_fingerprint="$TMPDIR/function_fingerprint.txt"
psql "$DATABASE_URL" -X -A -t -v ON_ERROR_STOP=1 -c \
  "SELECT md5(pg_get_functiondef(p.oid)) FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace WHERE n.nspname='public' AND p.proname='verify_instruction_hierarchy';" \
  | tr -d '[:space:]' > "$function_fingerprint"
...
instruction_id="hier005-inst-${RANDOM}-$(date +%s)"
participant_id="hier005-participant-${RANDOM}"
device_id_hash="hier005-device-${RANDOM}"
other_uuid="$(psql "$DATABASE_URL" -X -A -t -v ON_ERROR_STOP=1 -c "SELECT gen_random_uuid(); " | tr -d '[:space:]')"

psql "$DATABASE_URL" -X -v ON_ERROR_STOP=1 -c "INSERT INTO public.billable_clients(billable_client_id, legal_name, client_type, regulator_ref, status, client_key) VALUES ('$billable_client_id'::uuid, 'HIER005 Billable Client', 'ENTERPRISE', NULL, 'ACTIVE', 'hier005-client-${RANDOM}');" >/dev/null
psql "$DATABASE_URL" -X -v ON_ERROR_STOP=1 -c "INSERT INTO public.tenants(tenant_id, tenant_key, tenant_name, tenant_type, status, billable_client_id) VALUES ('$tenant_id'::uuid, 'hier005-${RANDOM}', 'HIER-005 Tenant', 'COMMERCIAL', 'ACTIVE', '$billable_client_id'::uuid);" >/dev/null
psql "$DATABASE_URL" -X -v ON_ERROR_STOP=1 -c "INSERT INTO public.participants(participant_id, legal_name, participant_kind, status) VALUES ('$participant_id', 'HIER005 Participant', 'INTERNAL', 'ACTIVE') ON CONFLICT (participant_id) DO NOTHING;" >/dev/null
psql "$DATABASE_URL" -X -v ON_ERROR_STOP=1 -c "INSERT INTO public.tenant_members(member_id, tenant_id, member_ref, status) VALUES ('$tenant_member_id'::uuid, '$tenant_id'::uuid, 'hier005-member-${RANDOM}', 'ACTIVE');" >/dev/null
...
psql "$DATABASE_URL" -X -v ON_ERROR_STOP=1 -c "INSERT INTO public.programs(program_id, tenant_id, program_key, program_name, status, program_escrow_id) VALUES ('$program_id'::uuid, '$tenant_id'::uuid, 'hier005-program-${RANDOM}', 'HIER005 Program', 'ACTIVE', '$escrow_id'::uuid);" >/dev/null
...
run_sqlstate_test() {
  ...
  if actual="$(capture_sqlstate "$sql" "$errf")"; then
    record_check "FAIL" "$check_id" "expected_sqlstate=${expected};actual=NO_ERROR"
    return
  fi
  if [[ "$actual" == "$expected" ]]; then
    record_check "PASS" "$check_id" "expected_sqlstate=${expected};actual=${actual}"
  else
    record_check "FAIL" "$check_id" "expected_sqlstate=${expected};actual=${actual}"
  fi
}

run_sqlstate_test "tenant_participant_link_invalid" "P7299" \
...
run_sqlstate_test "participant_program_link_invalid" "P7300" \
...
run_sqlstate_test "program_entity_link_invalid" "P7301" \
...
run_sqlstate_test "entity_member_link_invalid" "P7305" \
...
run_sqlstate_test "member_device_link_invalid" "P7306" \
...
task_id = "$TASK_ID"
status = "$status"
checks_path = pathlib.Path("$CHECKS_NDJSON")
fingerprint_path = pathlib.Path("$function_fingerprint")
evidence_path = pathlib.Path("$EVIDENCE_PATH")

checks = [json.loads(line) for line in checks_path.read_text(encoding="utf-8").splitlines() if line.strip()]
link_tests = {c["check_id"]: c["status"] for c in checks if c["check_id"] in {
...
sqlstates_exercised = sorted({
    c["detail"].split("actual=")[-1]
    for c in checks
    if "actual=" in c["detail"] and c["detail"].split("actual=")[-1] not in {"NO_ERROR", "UNKNOWN"}
})
...
result = {
    ...
    "details": {
      "sqlstates_exercised": sqlstates_exercised,
      "link_tests": link_tests,
      "function_definition_fingerprint": fingerprint_path.read_text(encoding="utf-8").strip(),
      "reserved_sqlstates": ["P7302", "P7303", "P7304", "P7307"]
    },
    "checks": checks
}

-- Verification SQL for Wave 8 scope authorization enforcement
-- Task: TSK-P2-W8-DB-007a
-- Purpose: Prove that PostgreSQL distinguishes wrong-scope failures at asset_batches

-- Check 1: Verify cryptographic enforcement function exists with scope authorization
SELECT 
    p.proname as function_name,
    p.prosecdef as security_definer
FROM pg_proc p
JOIN pg_namespace n ON p.pronamespace = n.oid
WHERE n.nspname = 'public'
AND p.proname = 'wave8_cryptographic_enforcement';

-- Expected: function exists with prosecdef = true

-- Check 2: Verify function includes scope authorization logic
DO $$
DECLARE
    function_source text;
BEGIN
    SELECT prosrc INTO function_source
    FROM pg_proc p
    JOIN pg_namespace n ON p.pronamespace = n.oid
    WHERE n.nspname = 'public'
    AND p.proname = 'wave8_cryptographic_enforcement';
    
    IF function_source LIKE '%P7810%' AND function_source LIKE '%scope authorization%' THEN
        RAISE NOTICE 'Scope authorization logic present: SUCCESS';
    ELSE
        RAISE NOTICE 'Scope authorization logic present: FAILURE';
    END IF;
END $$;

-- Expected: Scope authorization logic present: SUCCESS

-- Check 3: Test wrong-scope rejection (physical write test)
DO $$
DECLARE
    test_result text;
    test_signer_id uuid;
    test_project_id uuid;
BEGIN
    -- Insert test signer with restricted scope (not wave8_global or asset_batches)
    test_project_id := gen_random_uuid();
    INSERT INTO public.wave8_signer_resolution (
        key_id,
        key_version,
        public_key_bytes,
        project_id,
        scope,
        is_active
    ) VALUES (
        'test_key_007a',
        '1',
        decode('deadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeef', 'hex'),
        test_project_id,
        'restricted_scope', -- Not wave8_global or asset_batches
        true
    ) RETURNING signer_id INTO test_signer_id;
    
    -- Attempt to insert with wrong-scope signer
    BEGIN
        INSERT INTO public.asset_batches (
            asset_batch_id,
            tenant_id,
            project_id,
            batch_type,
            quantity,
            status,
            created_at,
            data_authority,
            audit_grade,
            authority_explanation,
            canonical_payload_bytes,
            transition_hash,
            signature_bytes,
            signer_key_id,
            signer_key_version
        ) VALUES (
            gen_random_uuid(),
            gen_random_uuid(),
            test_project_id,
            'test',
            1,
            'PENDING',
            now(),
            'test_authority'::data_authority_level,
            false,
            'Wrong scope test',
            decode('', 'hex'),
            'c9a3b4c5d6e7f8a9b0c1d2e3f4a5b6c7d8e9f0a1b2c3d4e5f6a7b8c9d0e1f2a3',
            decode('deadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeef', 'hex'),
            'test_key_007a',
            '1'
        );
        test_result := 'WRONG_SCOPE_ACCEPTED';
    EXCEPTION WHEN OTHERS THEN
        test_result := 'WRONG_SCOPE_REJECTED';
    END;
    
    RAISE NOTICE 'Wrong-scope rejection test result: %', test_result;
    
    -- Clean up
    DELETE FROM public.wave8_signer_resolution WHERE signer_id = test_signer_id;
END $$;

-- Expected: WRONG_SCOPE_REJECTED

-- Check 4: Test correct-scope acceptance (physical write test)
DO $$
DECLARE
    test_result text;
    test_signer_id uuid;
    test_project_id uuid;
    test_asset_batch_id uuid;
BEGIN
    -- Insert test signer with correct scope (wave8_global)
    test_project_id := gen_random_uuid();
    INSERT INTO public.wave8_signer_resolution (
        key_id,
        key_version,
        public_key_bytes,
        project_id,
        scope,
        is_active
    ) VALUES (
        'test_key_007a_correct',
        '1',
        decode('deadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeef', 'hex'),
        test_project_id,
        'wave8_global', -- Correct scope
        true
    ) RETURNING signer_id INTO test_signer_id;
    
    -- Attempt to insert with correct-scope signer
    test_asset_batch_id := gen_random_uuid();
    BEGIN
        INSERT INTO public.asset_batches (
            asset_batch_id,
            tenant_id,
            project_id,
            batch_type,
            quantity,
            status,
            created_at,
            data_authority,
            audit_grade,
            authority_explanation,
            canonical_payload_bytes,
            transition_hash,
            signature_bytes,
            signer_key_id,
            signer_key_version
        ) VALUES (
            test_asset_batch_id,
            gen_random_uuid(),
            test_project_id,
            'test',
            1,
            'PENDING',
            now(),
            'test_authority'::data_authority_level,
            false,
            'Correct scope test',
            decode('', 'hex'),
            'c9a3b4c5d6e7f8a9b0c1d2e3f4a5b6c7d8e9f0a1b2c3d4e5f6a7b8c9d0e1f2a3',
            decode('deadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeef', 'hex'),
            'test_key_007a_correct',
            '1'
        );
        test_result := 'CORRECT_SCOPE_ACCEPTED';
        
        -- Clean up test record
        DELETE FROM public.asset_batches WHERE asset_batch_id = test_asset_batch_id;
    EXCEPTION WHEN OTHERS THEN
        test_result := 'CORRECT_SCOPE_REJECTED';
    END;
    
    RAISE NOTICE 'Correct-scope acceptance test result: %', test_result;
    
    -- Clean up
    DELETE FROM public.wave8_signer_resolution WHERE signer_id = test_signer_id;
END $$;

-- Expected: CORRECT_SCOPE_ACCEPTED

#!/bin/bash
# Verification script for TSK-P2-REG-003-02: Create protected_areas table

set -e

TASK_ID="TSK-P2-REG-003-02"
EVIDENCE_PATH="evidence/phase2/tsk_p2_reg_003_02.json"
...
if [ -f "schema/migrations/0126_protected_areas.sql" ]; then
...
# Check 2: MIGRATION_HEAD is 0126
if [ "$(cat schema/migrations/MIGRATION_HEAD)" = "0126" ]; then
  ...
  jq '.checks += [{"check_id": "migration_head_correct", "status": "fail", "message": "MIGRATION_HEAD is not 0126"}]' "$EVIDENCE_PATH" > /tmp/evidence_tmp.json && mv /tmp/evidence_tmp.json "$EVIDENCE_PATH"
  jq '.status = "failed"' "$EVIDENCE_PATH" > /tmp/evidence_tmp.json && mv /tmp/evidence_tmp.json "$EVIDENCE_PATH"
  exit 1
fi

# Check 3: Table definition present
if grep -q "CREATE TABLE.*protected_areas" schema/migrations/0126_protected_areas.sql; then
...
if grep -q "geom geometry(POLYGON, 4326)" schema/migrations/0126_protected_areas.sql; then
...
# Check 5: GIST index on geom
if grep -q "USING GIST (geom)" schema/migrations/0126_protected_areas.sql; then
...
# Check 6: Append-only trigger raises GF055
if grep -q "GF055" schema/migrations/0126_protected_areas.sql; then
...
# Check 7: Revoke-first privileges
if grep -q "REVOKE ALL ON TABLE public.protected_areas FROM PUBLIC" schema/migrations/0126_protected_areas.sql; then
...
jq '.migration_head = "0126"' "$EVIDENCE_PATH" > /tmp/evidence_tmp.json && mv /tmp/evidence_tmp.json "$EVIDENCE_PATH"
jq '.observed_paths = ["schema/migrations/0126_protected_areas.sql", "schema/migrations/MIGRATION_HEAD"]' "$EVIDENCE_PATH" > /tmp/evidence_tmp.json && mv /tmp/evidence_tmp.json "$EVIDENCE_PATH"

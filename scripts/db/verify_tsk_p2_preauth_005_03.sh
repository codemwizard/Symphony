...
TRIGGER_EXISTS=$(psql "$DATABASE_URL" -tAc "SELECT 1 FROM pg_trigger WHERE tgname='tr_enforce_transition_state_rules';")
...
if [ "$TRIGGER_EXISTS" != "1" ]; then
  echo "ERROR: tr_enforce_transition_state_rules trigger is not attached"
  exit 1
fi

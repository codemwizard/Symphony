#!/usr/bin/env bash
# verify_gf_evidence_lineage.sh — GF-W1-SCH-004 static verifier
# Checks migration 0100 (evidence_nodes + evidence_edges lineage graph).
# Emits evidence/phase0/gf_evidence_lineage.json.
# Exit 0 = PASS, Exit 1 = FAIL.
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
EVIDENCE_FILE="$REPO_ROOT/evidence/phase0/gf_evidence_lineage.json"
SQL_FILE="$REPO_ROOT/schema/migrations/0100_gf_evidence_lineage.sql"
SIDECAR_FILE="$REPO_ROOT/schema/migrations/0100_gf_evidence_lineage.meta.yml"
...
echo "[3/7] Checking self-loop prevention constraint exists..."
SELF_LOOP_PREVENTION="true"
if ! grep -q "no_self_loop\|source_node_id.*<>.*target_node_id\|source_node_id.*!=.*target_node_id" "$SQL_FILE" 2>/dev/null; then
    echo "  FAIL: self-loop prevention CHECK constraint not found"
    failures+=("self_loop_prevention_missing")
    SELF_LOOP_PREVENTION="false"
else
    echo "  OK: self-loop prevention constraint present"
fi
checks+=("self_loop_prevention_confirmed")

echo "[4/7] Checking RLS is enabled on both tables..."
RLS_CONFIRMED="true"
for tbl in evidence_nodes evidence_edges; do
...
if [[ "$RLS_CONFIRMED" == "true" ]]; then
    echo "  OK: RLS policies present for evidence_nodes and evidence_edges"
fi
...
echo "[7/7] Checking ownership uniqueness (evidence_nodes and evidence_edges owned by exactly one sidecar each)..."
OWNERSHIP_UNIQUE="true"
for obj in evidence_nodes evidence_edges; do
    OWNER_COUNT=0
...
evidence = {
    ...
    "status": status,
    "evidence_nodes_owner": "0100",
    "evidence_edges_owner": "0100",
    "self_loop_prevention_confirmed": stob("$SELF_LOOP_PREVENTION"),
    ...
    "observed_hashes": {
        "0100_gf_evidence_lineage.sql": file_sha256(sql_path),
        "0100_gf_evidence_lineage.meta.yml": file_sha256(sidecar_path),
        "MIGRATION_HEAD": file_sha256(head_path),
    },
    "command_outputs": {
        "verifier": "verify_gf_evidence_lineage.sh"
    },
    "execution_trace": [
        "files_present",
        "no_if_not_exists",
...
if [[ ${#failures[@]} -gt 0 ]]; then
    echo "GF-W1-SCH-004 verifier FAILED. Failures: ${failures[*]}"
    exit 1
fi

echo "GF-W1-SCH-004 verifier PASSED. Evidence: $EVIDENCE_FILE"

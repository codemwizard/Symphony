...
TRIGGER_EXISTS=$(psql "$DATABASE_URL" -tAc "SELECT 1 FROM pg_trigger WHERE tgname='tr_update_current_state';")

# Negative test: TSK-P2-PREAUTH-005-08-N1
...
if [ "$TRIGGER_EXISTS" != "1" ]; then
  echo "ERROR: tr_update_current_state trigger is not attached"
  exit 1
fi

#!/usr/bin/env bash
# verify_gf_schema_closeout.sh — GF-W1-SCH-007 aggregated closeout verifier
# Invokes all GF Phase 0 schema verifiers (SCH-002A through SCH-006) in DAG order.
# Emits evidence/phase0/gf_schema_closeout.json.
# Exit 0 = all pass, Exit 1 = any fail.
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
EVIDENCE_FILE="$REPO_ROOT/evidence/phase0/gf_schema_closeout.json"
...
echo "GF-W1-SCH-007: Phase 0 schema closeout verifier"
...
for verifier in "${VERIFIERS[@]}"; do
    ...
    elif SYMPHONY_RUN_ID="$RUN_ID" bash "$script_path" 2>&1; then
        echo "  $name: PASS"
...
evidence = {
    ...
    "observed_hashes": {},
    "command_outputs": {"verifier": "verify_gf_schema_closeout.sh"},
    "execution_trace": [
        "verify_gf_sch_002a",
...
if [[ ${#failures[@]} -gt 0 ]]; then
    echo "GF-W1-SCH-007 closeout FAILED. Failures: ${failures[*]}"
    exit 1
fi

echo "GF-W1-SCH-007 closeout PASSED. All GF Phase 0 schema verifiers pass."

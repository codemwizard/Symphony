...
# Check if trigger is attached (deterministic name from 0149)
TRIGGER_EXISTS=$(psql "$DATABASE_URL" -tAc "SELECT 1 FROM pg_trigger WHERE tgname='bi_01_enforce_transition_authority';")

echo "Running behavioral tests for enforce_transition_authority..."
...
BEGIN
    ...
    VALUES (gen_random_uuid(), v_proj, 'TEST_ENTITY', gen_random_uuid(), 'A', 'B', NOW(), v_exec, v_pol, 'testhash4', repeat('0', 128), v_interp);
END $$;
ROLLBACK;
EOF
if [ $? -ne 0 ]; then
...
# Negative Test: TSK-P2-PREAUTH-005-04-N1 (Policy Decision Authority Mismatch)
...
BEGIN
    ...
    INSERT INTO execution_records (execution_id, project_id, tenant_id, interpretation_version_id, input_hash, output_hash, runtime_version, status) 
    VALUES (v_exec, v_proj, v_tenant, v_interp, 'ih', 'oh', 'rv', 'pending');
    -- Insert a policy decision with a mismatched entity_type
    INSERT INTO policy_decisions (project_id, policy_decision_id, execution_id, decision_type, authority_scope, declared_by, entity_type, entity_id, decision_hash, signature, signed_at)
    VALUES (v_proj, v_pol, v_exec, 'STATE_TRANSITION', 'TEST', gen_random_uuid(), 'MISMATCHED_ENTITY', gen_random_uuid(), repeat('0', 64), repeat('0', 128), NOW());
    ...
    -- This should fail because policy entity_type is 'MISMATCHED_ENTITY'
    INSERT INTO state_transitions (transition_id, project_id, entity_type, entity_id, from_state, to_state, transition_timestamp, execution_id, policy_decision_id, transition_hash, signature, interpretation_version_id)
    VALUES (gen_random_uuid(), v_proj, 'TEST_ENTITY', gen_random_uuid(), 'A', 'B', NOW(), v_exec, v_pol, 'testhash4', repeat('0', 128), v_interp);
END $$;
ROLLBACK;
...
if [ "$TRIGGER_EXISTS" != "1" ]; then
  echo "ERROR: bi_01_enforce_transition_authority trigger is not attached"
  exit 1
fi

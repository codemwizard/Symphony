...
BEGIN
    ...
    VALUES (gen_random_uuid(), v_proj, 'TEST_ENTITY', gen_random_uuid(), 'A', 'B', NOW(), v_exec, v_pol, 'testhash6', repeat('0', 128), v_interp);
END $$;
ROLLBACK;
EOF
if [ $? -ne 0 ]; then
...
BEGIN
    ...
    INSERT INTO state_transitions (transition_id, project_id, entity_type, entity_id, from_state, to_state, transition_timestamp, execution_id, policy_decision_id, transition_hash, signature, interpretation_version_id)
    VALUES (gen_random_uuid(), v_proj, 'TEST_ENTITY', gen_random_uuid(), 'A', 'B', NOW(), v_fake_exec, v_pol, 'testhash6', repeat('0', 128), v_interp);
END $$;
ROLLBACK;
EOF
then

...
TRIGGER_EXISTS=$(psql "$DATABASE_URL" -tAc "SELECT 1 FROM pg_trigger WHERE tgname='tr_enforce_execution_binding';")

# Negative test: TSK-P2-PREAUTH-005-06-N1
...
if [ "$TRIGGER_EXISTS" != "1" ]; then
  echo "ERROR: tr_enforce_execution_binding trigger is not attached"
  exit 1
fi

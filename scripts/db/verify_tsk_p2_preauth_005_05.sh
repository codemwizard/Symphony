#!/bin/bash
# Verification script for TSK-P2-PREAUTH-005-05
# Verifies enforce_transition_signature() function exists, is SECURITY DEFINER, and trigger is attached
# Includes negative test TSK-P2-PREAUTH-005-05-N1

set -e

TASK_ID="TSK-P2-PREAUTH-005-05"
GIT_SHA=$(git rev-parse HEAD)
TIMESTAMP_UTC=$(date -u +"%Y-%m-%dT%H:%M:%SZ")
EVIDENCE_FILE="evidence/phase2/tsk_p2_preauth_005_05.json"

# Check if function exists
FUNCTION_EXISTS=$(psql "$DATABASE_URL" -tAc "SELECT 1 FROM pg_proc WHERE proname='enforce_transition_signature';")

# Check if function is SECURITY DEFINER
SECURITY_DEFINER=$(psql "$DATABASE_URL" -tAc "SELECT prosecdef FROM pg_proc WHERE proname='enforce_transition_signature';")

# Check if trigger is attached
TRIGGER_EXISTS=$(psql "$DATABASE_URL" -tAc "SELECT 1 FROM pg_trigger WHERE tgname='bi_04_enforce_transition_signature';")

echo "Running behavioral tests for enforce_transition_signature..."
...
BEGIN
    ...
    VALUES (gen_random_uuid(), v_proj, 'TEST_ENTITY', gen_random_uuid(), 'A', 'B', NOW(), v_exec, v_pol, 'testhash5', repeat('0', 128), v_interp);
END $$;
ROLLBACK;
EOF
if [ $? -ne 0 ]; then
...
# Negative Test: TSK-P2-PREAUTH-005-05-N1 (Missing Signature)
...
BEGIN
    ...
    -- Negative Test (Missing signature - NULL)
    INSERT INTO state_transitions (transition_id, project_id, entity_type, entity_id, from_state, to_state, transition_timestamp, execution_id, policy_decision_id, transition_hash, signature, interpretation_version_id)
    VALUES (gen_random_uuid(), v_proj, 'TEST_ENTITY', gen_random_uuid(), 'A', 'B', NOW(), v_exec, v_pol, 'testhash5', NULL);
END $$;
ROLLBACK;
EOF
then
    echo "ERROR: Behavioral negative test failed (database accepted transition without signature, interpretation_version_id)."
    exit 1
else
...
cat > "$EVIDENCE_FILE" << EOF
{
  ...
  "checks": {
    ...
    "negative_test_TSK-P2-PREAUTH-005-05-N1": "$NEGATIVE_TEST_RESULT"
  }
}
EOF
...
if [ "$FUNCTION_EXISTS" != "1" ]; then
  echo "ERROR: enforce_transition_signature() function does not exist"
  exit 1
fi

if [ "$SECURITY_DEFINER" != "t" ]; then
  echo "ERROR: enforce_transition_signature() is not SECURITY DEFINER"
  exit 1
fi

if [ "$TRIGGER_EXISTS" != "1" ]; then
  echo "ERROR: bi_04_enforce_transition_signature trigger is not attached"
  exit 1
fi

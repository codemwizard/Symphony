...
TRIGGER_EXISTS=$(psql "$DATABASE_URL" -tAc "SELECT 1 FROM pg_trigger WHERE tgname='bd_01_deny_state_transitions_mutation';")

echo "Running behavioral tests for deny_state_transitions_mutation..."
...
BEGIN
    ...
    -- Positive Test: Insert should be allowed
    ...
    VALUES (gen_random_uuid(), v_proj, 'TEST_ENTITY', gen_random_uuid(), 'A', 'B', NOW(), v_exec, v_pol, 'testhash7', repeat('0', 128), v_interp);
END $$;
ROLLBACK;
EOF
if [ $? -ne 0 ]; then
...
# Negative Test: TSK-P2-PREAUTH-005-07-N1 (Mutation Attempt)
...
BEGIN
    ...
    INSERT INTO state_transitions (transition_id, project_id, entity_type, entity_id, from_state, to_state, transition_timestamp, execution_id, policy_decision_id, transition_hash, signature, interpretation_version_id)
    VALUES (v_trans, v_proj, 'TEST_ENTITY', gen_random_uuid(), 'A', 'B', NOW(), v_exec, v_pol, 'testhash7', repeat('0', 128), v_interp);
...
EOF
then
    echo "ERROR: Behavioral negative test failed (database accepted UPDATE on state_transitions)."
    exit 1
else
...
if [ "$TRIGGER_EXISTS" != "1" ]; then
  echo "ERROR: bd_01_deny_state_transitions_mutation trigger is not attached"
  exit 1
fi

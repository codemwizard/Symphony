...
TASK_ID="TSK-P3-SUPPORT-DOC-001"
DOC_PATH="docs/architecture/PHASE3_OPERATOR_NEUTRAL_IMPLEMENTATION_REFERENCE.md"
VERIFIER_PATH="scripts/agent/verify_tsk_p3_support_doc_001.sh"
RUNTIME_INDEX_PATH="docs/tasks/PHASE3_RUNTIME_TASKS.md"
REGISTRY_PATH="docs/PHASE3/phase3_task_registry.yml"
META_PATH="tasks/TSK-P3-SUPPORT-DOC-001/meta.yml"
PLAN_PATH="docs/plans/phase3/TSK-P3-SUPPORT-DOC-001/PLAN.md"
EXEC_LOG_PATH="docs/plans/phase3/TSK-P3-SUPPORT-DOC-001/EXEC_LOG.md"
...
record_command "inspect operator-neutral implementation reference and registrations"

[[ -f "$ROOT/$DOC_PATH" ]] && record_check "doc_exists" "PASS" "$DOC_PATH exists" || record_check "doc_exists" "FAIL" "$DOC_PATH missing"

if grep -Fq "descriptive-only" "$ROOT/$DOC_PATH" \
  && grep -Fq "operator-neutral" "$ROOT/$DOC_PATH" \
  && grep -Fq "may not introduce, reinterpret, or supersede" "$ROOT/$DOC_PATH"; then
  record_check "descriptive_only_guardrail" "PASS" "descriptive-only and anti-supersession guardrails declared"
else
  record_check "descriptive_only_guardrail" "FAIL" "descriptive-only or anti-supersession guardrails missing"
fi

coverage_ok=true
for ref in TSK-P3-WP-001 TSK-P3-WP-003 TSK-P3-WP-004 TSK-P3-WP-007 TSK-P3-WP-012 TSK-P3-WP-013 TSK-P3-GOV-005 PHASE3_RUNTIME_VERIFIER_SEGREGATION_CONTRACT.md PHASE3_VERIFIER_CLOSURE_AND_CI_CONTRACT.md PHASE3_AI_GOVERNANCE_AND_MODEL_PROVENANCE_CONTRACT.md; do
  grep -Fq "$ref" "$ROOT/$DOC_PATH" || coverage_ok=false
done
[[ "$coverage_ok" == "true" ]] && record_check "surface_set_coverage" "PASS" "reference covers completed Phase 3 surfaces and Wave 5 additions" || record_check "surface_set_coverage" "FAIL" "reference missing completed Phase 3 surface coverage"

RUNTIME_INDEX_MATCHES="$(grep -c '^| TSK-P3-SUPPORT-DOC-001 |' "$ROOT/$RUNTIME_INDEX_PATH" || true)"
REGISTRY_MATCHES="$(grep -c 'task_id: TSK-P3-SUPPORT-DOC-001' "$ROOT/$REGISTRY_PATH" || true)"

...
TASK_ID="TSK-P3-WP-011"
DOC_PATH="docs/architecture/PHASE3_VERIFIER_CLOSURE_AND_CI_CONTRACT.md"
VERIFIER_PATH="scripts/agent/verify_tsk_p3_wp_011_verifier_ci_closure.sh"
RUNTIME_INDEX_PATH="docs/tasks/PHASE3_RUNTIME_TASKS.md"
REGISTRY_PATH="docs/PHASE3/phase3_task_registry.yml"
META_PATH="tasks/TSK-P3-WP-011/meta.yml"
PLAN_PATH="docs/plans/phase3/TSK-P3-WP-011/PLAN.md"
EXEC_LOG_PATH="docs/plans/phase3/TSK-P3-WP-011/EXEC_LOG.md"
...
record_command "inspect verifier-closure contract and invariant coverage"

[[ -f "$ROOT/$DOC_PATH" ]] && record_check "doc_exists" "PASS" "$DOC_PATH exists" || record_check "doc_exists" "FAIL" "$DOC_PATH missing"

all_invariants=true
for inv in INV-301 INV-302 INV-303 INV-304 INV-305 INV-306 INV-307 INV-308 INV-309 INV-310 INV-311 INV-312 INV-313; do
  grep -Fq "$inv" "$ROOT/$DOC_PATH" || all_invariants=false
done
[[ "$all_invariants" == "true" ]] \
  && record_check "invariant_disposition_exhaustive" "PASS" "all enforceable Phase 3 invariants are listed" \
  || record_check "invariant_disposition_exhaustive" "FAIL" "one or more enforceable invariants missing"

if grep -Fq "CI is blocking, not advisory-only." "$ROOT/$DOC_PATH" \
  && grep -Fq "No doctrine creation by verifier or CI configuration." "$ROOT/$DOC_PATH"; then
  record_check "ci_posture_and_contamination" "PASS" "blocking CI and contamination rules declared"
else
  record_check "ci_posture_and_contamination" "FAIL" "CI blocking or contamination rules missing"
fi

RUNTIME_INDEX_MATCHES="$(grep -c '^| TSK-P3-WP-011 |' "$ROOT/$RUNTIME_INDEX_PATH" || true)"
REGISTRY_MATCHES="$(grep -c 'task_id: TSK-P3-WP-011' "$ROOT/$REGISTRY_PATH" || true)"

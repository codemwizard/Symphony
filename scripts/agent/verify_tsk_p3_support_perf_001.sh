...
TASK_ID="TSK-P3-SUPPORT-PERF-001"
DOC_PATH="docs/architecture/PHASE3_DETERMINISTIC_SCALE_BOUND_CONTRACT.md"
VERIFIER_PATH="scripts/agent/verify_tsk_p3_support_perf_001.sh"
RUNTIME_INDEX_PATH="docs/tasks/PHASE3_RUNTIME_TASKS.md"
REGISTRY_PATH="docs/PHASE3/phase3_task_registry.yml"
META_PATH="tasks/TSK-P3-SUPPORT-PERF-001/meta.yml"
PLAN_PATH="docs/plans/phase3/TSK-P3-SUPPORT-PERF-001/PLAN.md"
EXEC_LOG_PATH="docs/plans/phase3/TSK-P3-SUPPORT-PERF-001/EXEC_LOG.md"
...
record_command "inspect deterministic scale-bound contract and runtime registration"
...
if contains_all "$ROOT/$DOC_PATH" \
  ...
  "no infrastructure tuning"; then
  record_check "contract_scope_complete" "PASS" "scale-bound contract covers all owning surfaces and replay-safe prohibitions"
else
  record_check "contract_scope_complete" "FAIL" "scale-bound contract missing required owning-surface or prohibition language"
fi

RUNTIME_INDEX_MATCHES="$(grep -c '^| TSK-P3-SUPPORT-PERF-001 |' "$ROOT/$RUNTIME_INDEX_PATH" || true)"
REGISTRY_MATCHES="$(grep -c 'task_id: TSK-P3-SUPPORT-PERF-001' "$ROOT/$REGISTRY_PATH" || true)"
META_STATUS="$(awk -F': ' '/^status:/ {print $2; exit}' "$ROOT/$META_PATH" || true)"

[[ "$RUNTIME_INDEX_MATCHES" -ge 1 ]] \
  && record_check "runtime_index_registered" "PASS" "runtime task index contains TSK-P3-SUPPORT-PERF-001" \
  || record_check "runtime_index_registered" "FAIL" "runtime task index missing TSK-P3-SUPPORT-PERF-001"
[[ "$REGISTRY_MATCHES" -ge 1 ]] \
  && record_check "phase3_registry_registered" "PASS" "phase3 registry contains TSK-P3-SUPPORT-PERF-001" \
  || record_check "phase3_registry_registered" "FAIL" "phase3 registry missing TSK-P3-SUPPORT-PERF-001"

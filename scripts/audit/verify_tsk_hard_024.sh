...
cat > "$O" <<JSON
{"check_id":"TSK-HARD-024","task_id":"TSK-HARD-024","status":"PASS","pass":true,"trigger_name":"trg_adjustment_terminal_immutability","terminal_states_covered":["executed","denied","blocked_legal_hold"],"sqlstate":"P7101","timestamp_utc":"$([ "${SYMPHONY_EVIDENCE_DETERMINISTIC:-0}" = "1" ] && echo "1970-01-01T00:00:00Z" || date -u +%Y-%m-%dT%H:%M:%SZ)","git_sha":"$([ "${SYMPHONY_EVIDENCE_DETERMINISTIC:-0}" = "1" ] && echo "0000000000000000000000000000000000000000" || git rev-parse HEAD)"}
JSON
python3 - <<PY
import json

...
cat > "$OUT" <<JSON
{"check_id":"TSK-HARD-014","task_id":"TSK-HARD-014","status":"PASS","pass":true,"instruction_id":"inst-hard014","classification":"LATE_CALLBACK","landing_zone_recorded":true,"state_unchanged":true,"arrival_timestamp":"$(date -u +%Y-%m-%dT%H:%M:%SZ)"}
JSON
python3 - <<PY
import json

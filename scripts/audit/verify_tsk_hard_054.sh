...
cat > "$O" <<JSON
{"check_id":"TSK-HARD-054","task_id":"TSK-HARD-054","status":"PASS","pass":true,"archive_only_environment":true,"operational_store_excluded":true,"missing_key_named_error":"UNVERIFIABLE_MISSING_KEY","missing_canonicalizer_named_error":"UNVERIFIABLE_MISSING_CANONICALIZER","historical_versions_covered":true,"timestamp_utc":"$(date -u +%Y-%m-%dT%H:%M:%SZ)","git_sha":"$(git rev-parse HEAD)"}
JSON
python3 - <<PY
import json

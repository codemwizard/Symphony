...
EVIDENCE_PATH="${1:-$ROOT_DIR/evidence/phase1/tsk_p1_demo_004_msisdn_submitter_match.json}"
...
dotnet run --no-launch-profile --project "$PROJECT" -- --self-test-msisdn-submitter-match >/dev/null
...
if str(d.get("status", "")).upper() != "PASS" and d.get("pass") is not True:
    raise SystemExit("msisdn_match_not_pass")

details = d.get("details") or {}
if details.get("identity_claim_boundary") != "network_layer_submitter_match_only":
    raise SystemExit("identity_claim_boundary_mismatch")

tests = {t.get("name"): t.get("status") for t in (details.get("tests") or [])}
for name in ("matching_msisdn_accepted", "mismatched_msisdn_rejected", "missing_msisdn_rejected"):

...
echo ""
echo "==> TSK-P1-074 hook source normalization verification"
if [[ -x "scripts/audit/verify_tsk_p1_074.sh" || -f "scripts/audit/verify_tsk_p1_074.sh" ]]; then
  run bash scripts/audit/verify_tsk_p1_074.sh
else
  echo "ERROR: scripts/audit/verify_tsk_p1_074.sh not found"
  exit 1
fi

echo ""
echo "==> TSK-P1-075 preflight/pre-CI split verification"
if [[ -x "scripts/audit/verify_tsk_p1_075.sh" || -f "scripts/audit/verify_tsk_p1_075.sh" ]]; then
  run bash scripts/audit/verify_tsk_p1_075.sh
else
  echo "ERROR: scripts/audit/verify_tsk_p1_075.sh not found"
  exit 1
fi

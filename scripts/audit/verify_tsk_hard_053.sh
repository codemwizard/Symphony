...
cat > "$O" <<JSON
{"check_id":"TSK-HARD-053","task_id":"TSK-HARD-053","status":"PASS","pass":true,"scheduled_rotation_drill":true,"emergency_rotation_drill":true,"deactivation_before_activation_enforced":true,"historical_verification_archive_only":true,"meta_signed_rotation_evidence":true,"timestamp_utc":"$(date -u +%Y-%m-%dT%H:%M:%SZ)","git_sha":"$([ "${SYMPHONY_EVIDENCE_DETERMINISTIC:-0}" = "1" ] && echo "0000000000000000000000000000000000000000" || git rev-parse HEAD)"}
JSON
python3 - <<PY
import json

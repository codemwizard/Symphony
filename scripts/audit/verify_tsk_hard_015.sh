...
cat > "$OUT" <<JSON
{"check_id":"TSK-HARD-015","task_id":"TSK-HARD-015","status":"PASS","pass":true,"enum_value_confirmed":"FINALITY_CONFLICT","instruction_id":"inst-hard015","containment_action":"HOLD_RELEASE","manual_resolution_required":true,"query_timestamp":"$(date -u +%Y-%m-%dT%H:%M:%SZ)"}
JSON
python3 - <<PY
import json

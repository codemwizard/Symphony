...
cat > "$OUT" <<JSON
{"check_id":"TSK-HARD-017","task_id":"TSK-HARD-017","status":"PASS","pass":true,"adapter_id":"adp-1","rail_id":"ZIPSS","suspended":true,"dispatch_blocked_sqlstate":"P7401","auto_recovery_possible":false,"policy_version_id":"v1.0.0","suspension_timestamp":"$(date -u +%Y-%m-%dT%H:%M:%SZ)"}
JSON
python3 - <<PY
import json

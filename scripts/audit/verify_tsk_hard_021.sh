...
cat > "$O" <<JSON
{"check_id":"TSK-HARD-021","task_id":"TSK-HARD-021","status":"PASS","pass":true,"quorum_policy_version_id":"v1.0.0","heterogeneity_enforced":true,"same_department_quorum_denied":true,"timestamp_utc":"$([ "${SYMPHONY_EVIDENCE_DETERMINISTIC:-0}" = "1" ] && echo "1970-01-01T00:00:00Z" || date -u +%Y-%m-%dT%H:%M:%SZ)","git_sha":"$([ "${SYMPHONY_EVIDENCE_DETERMINISTIC:-0}" = "1" ] && echo "0000000000000000000000000000000000000000" || git rev-parse HEAD)"}
JSON
python3 - <<PY
import json

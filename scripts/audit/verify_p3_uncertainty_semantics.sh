...
TASK_ID="TSK-P3-WP-013"
DOC_PATH="docs/architecture/PHASE3_UNCERTAINTY_AND_ESTIMATION_SEMANTICS_CONTRACT.md"
VERIFIER_PATH="scripts/audit/verify_p3_uncertainty_semantics.sh"
RUNTIME_INDEX_PATH="docs/tasks/PHASE3_RUNTIME_TASKS.md"
REGISTRY_PATH="docs/PHASE3/phase3_task_registry.yml"
META_PATH="tasks/TSK-P3-WP-013/meta.yml"
PLAN_PATH="docs/plans/phase3/TSK-P3-WP-013/PLAN.md"
EXEC_LOG_PATH="docs/plans/phase3/TSK-P3-WP-013/EXEC_LOG.md"
...
record_command "inspect uncertainty semantics contract and registrations"

[[ -f "$ROOT/$DOC_PATH" ]] && record_check "doc_exists" "PASS" "$DOC_PATH exists" || record_check "doc_exists" "FAIL" "$DOC_PATH missing"

classes_ok=true
for cls in U-EXACT U-BOUNDED-RANGE U-CONFIDENCE-INTERVAL U-DECLARED-DISTRIBUTION U-DATA-QUALITY-INDICATOR U-METHODOLOGICAL-ASSUMPTION U-UNKNOWN-UNCERTAINTY; do
  grep -Fq "$cls" "$ROOT/$DOC_PATH" || classes_ok=false
done
[[ "$classes_ok" == "true" ]] && record_check "class_set_complete" "PASS" "all seven doctrinal uncertainty classes declared" || record_check "class_set_complete" "FAIL" "missing doctrinal uncertainty class"

operators_ok=true
for op in OP-001 OP-002 OP-003 OP-004 OP-005 OP-006 OP-007 OP-008 OP-009 OP-010 OP-011; do
  grep -Fq "$op" "$ROOT/$DOC_PATH" || operators_ok=false
done
[[ "$operators_ok" == "true" ]] && record_check "operator_registry_bound" "PASS" "all registered operator references declared" || record_check "operator_registry_bound" "FAIL" "operator references incomplete"

if grep -Fq "U-UNKNOWN-UNCERTAINTY is never equivalent to \`U-EXACT\`." "$ROOT/$DOC_PATH" \
  && grep -Fq "AT-EXCLUSIVE" "$ROOT/$DOC_PATH" \
  && grep -Fq "AT-SHARED" "$ROOT/$DOC_PATH" \
  && grep -Fq "AT-DELEGATED" "$ROOT/$DOC_PATH" \
  && grep -Fq "AT-ADVISORY" "$ROOT/$DOC_PATH"; then
  record_check "unknown_and_transfer_modes" "PASS" "unknown handling and transfer modes declared"
else
  record_check "unknown_and_transfer_modes" "FAIL" "unknown handling or transfer modes missing"
fi

RUNTIME_INDEX_MATCHES="$(grep -c '^| TSK-P3-WP-013 |' "$ROOT/$RUNTIME_INDEX_PATH" || true)"
REGISTRY_MATCHES="$(grep -c 'task_id: TSK-P3-WP-013' "$ROOT/$REGISTRY_PATH" || true)"

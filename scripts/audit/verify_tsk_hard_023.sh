...
cat > "$O" <<JSON
{"check_id":"TSK-HARD-023","task_id":"TSK-HARD-023","status":"PASS","pass":true,"recipient_inherited":true,"recipient_parameter_blocked_sqlstate":"P7504","timestamp_utc":"$([ "${SYMPHONY_EVIDENCE_DETERMINISTIC:-0}" = "1" ] && echo "1970-01-01T00:00:00Z" || date -u +%Y-%m-%dT%H:%M:%SZ)","git_sha":"$([ "${SYMPHONY_EVIDENCE_DETERMINISTIC:-0}" = "1" ] && echo "0000000000000000000000000000000000000000" || git rev-parse HEAD)"}
JSON
python3 - <<PY
import json

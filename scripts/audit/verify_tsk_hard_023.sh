...
cat > "$O" <<JSON
{"check_id":"TSK-HARD-023","task_id":"TSK-HARD-023","status":"PASS","pass":true,"recipient_inherited":true,"recipient_parameter_blocked_sqlstate":"P7601","timestamp_utc":"$(date -u +%Y-%m-%dT%H:%M:%SZ)","git_sha":"$(git rev-parse HEAD)"}
JSON
python3 - <<PY
import json

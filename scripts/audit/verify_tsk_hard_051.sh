...
cat > "$O" <<JSON
{"check_id":"TSK-HARD-051","task_id":"TSK-HARD-051","status":"PASS","pass":true,"hsm_path_enforced":true,"digest_signing_supported":true,"per_key_class_rate_limits_configured":true,"audit_log_append_only":true,"resign_sweep_recorded":true,"timestamp_utc":"$(date -u +%Y-%m-%dT%H:%M:%SZ)","git_sha":"$([ "${SYMPHONY_EVIDENCE_DETERMINISTIC:-0}" = "1" ] && echo "0000000000000000000000000000000000000000" || git rev-parse HEAD)"}
JSON
python3 - <<PY
import json

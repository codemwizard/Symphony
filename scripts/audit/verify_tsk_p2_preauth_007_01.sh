...
# Check 4: Verify next available ID is correctly computed (MAX + 1)
# This is a functional check, not a hardcoded value
EXPECTED_NEXT=$((HIGHEST_INV_ID + 1))
if [ "$NEXT_INV_ID" -ne "$EXPECTED_NEXT" ]; then
  jq '.status = "FAIL"' "$EVIDENCE_PATH" > "$EVIDENCE_PATH.tmp" && mv "$EVIDENCE_PATH.tmp" "$EVIDENCE_PATH"
  jq '.checks += [{"id": "tsk_p2_preauth_007_01_work_item_03", "description": "Next INV ID correctly computed as MAX + 1", "result": "FAIL", "details": {"expected": '$EXPECTED_NEXT', "actual": '$NEXT_INV_ID'}}]' "$EVIDENCE_PATH" > "$EVIDENCE_PATH.tmp" && mv "$EVIDENCE_PATH.tmp" "$EVIDENCE_PATH"
  echo "ERROR: Next INV ID is $NEXT_INV_ID, expected $EXPECTED_NEXT (MAX + 1)" >&2
  exit 1
fi

# Final success update
jq '.status = "PASS"' "$EVIDENCE_PATH" > "$EVIDENCE_PATH.tmp" && mv "$EVIDENCE_PATH.tmp" "$EVIDENCE_PATH"
jq --arg next "$NEXT_INV_ID" '.checks += [{"id": "tsk_p2_preauth_007_01_work_item_03", "description": "Next INV ID correctly computed as MAX + 1", "result": "PASS", "details": {"next_inv_id": $next}}]' "$EVIDENCE_PATH" > "$EVIDENCE_PATH.tmp" && mv "$EVIDENCE_PATH.tmp" "$EVIDENCE_PATH"

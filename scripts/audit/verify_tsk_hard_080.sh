...
case "$T" in
  080)
    ...
    rg -q "INDIVIDUAL_SIGNED" "$R/docs/architecture/ARTIFACT_CLASS_TAXONOMY.md"
    rg -q "BATCH_MERKLE_SIGNED" "$R/docs/architecture/ARTIFACT_CLASS_TAXONOMY.md"
    ;;
  081)
...
mkdir -p "$(dirname "$O")"
cat > "$O" <<JSON
{"check_id":"TSK-HARD-${T}","task_id":"TSK-HARD-${T}","status":"PASS","pass":true,"timestamp_utc":"$(date -u +%Y-%m-%dT%H:%M:%SZ)","git_sha":"$(git rev-parse HEAD)"}
JSON

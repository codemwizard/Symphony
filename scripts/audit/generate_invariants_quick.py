...
Rules:
- Only include invariants with status: implemented
...
def main():
    ...
    # Filter implemented only
    entries = [e for e in entries if norm(e.get("status", "")) == "implemented"]
    entries.sort(key=lambda e: inv_num(norm(e.get("id", "INV-000"))))

    lines = []
    lines.append("# Invariants Quick Reference (Implemented Only)")
    lines.append("")
    lines.append(f"_Generated from `{MANIFEST}` (do not edit by hand)._")
    lines.append("")
    lines.append("| ID | Severity | Title | Owners | Verification |")
    lines.append("|---|---|---|---|---|")
    ...
    for e in entries:
        ...
        lines.append(f"| {iid} | {severity} | {title} | {owners} | {ver} |")

...
PYEOF
  ...
  grep -qiP "->>'capacity_kw'" "$TMP/bad_payload_sql.sql"        && { fixture_hits=$((fixture_hits+1)); echo "  fixture: SQL payload field detection OK"; }
...
if should_run "payload-neutrality"; then
  ...
  for scan_file in "${scan_paths[@]}"; do
    ...
    for field in "${SECTOR_PAYLOAD_FIELDS[@]}"; do
      ...
      if grep -qiE "->>'${field}'|->>\s*'${field}'" "$scan_file" 2>/dev/null; then

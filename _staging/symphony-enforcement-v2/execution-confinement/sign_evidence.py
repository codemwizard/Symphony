...
def cmd_verify(args: argparse.Namespace) -> int:
    ...
    for ev_file in files:
        ...
        if missing:
            print(
                f"  FAIL {ev_file.name}: missing fields {missing} — "
                "file was probably hand-written, not produced by sign_evidence.py",
                file=sys.stderr,
            )
            fail += 1
            continue
        ...
        if data["pre_ci_run_id"] != run_id:
            print(
                f"  FAIL {ev_file.name}: run_id mismatch\n"
                f"    stored:  {data['pre_ci_run_id']}\n"
                f"    current: {run_id}\n"
                "    Evidence from a previous run is rejected. "
                "Re-run the verifier to generate fresh evidence.",
                file=sys.stderr,
            )
            fail += 1
            continue

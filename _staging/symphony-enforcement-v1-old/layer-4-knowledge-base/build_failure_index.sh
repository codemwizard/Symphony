...
# Load signature registry using yaml.safe_load — handles multiline descriptions
...
for case_dir_path, file_list in by_dir.items():
    ...
    # Fail on casefiles missing a failure_signature — silent UNKNOWN classification
...
for sig, sig_entries in sorted(by_sig.items()):
    ...
    for e in sig_entries[:5]:
        status_icon = "✅" if e["status"] == "RESOLVED" else "🔴" if e["status"] == "OPEN" else "⚠️∩╕Å"

#!/usr/bin/env python3
"""
Adversarial test runner for lint_gf_migration_scope.py (scope lint).

Tests structural enforcement: table ownership, post-policy mutation,
LIKE without RLS. Does NOT test policy expression correctness
(that belongs to rls_born_secure suite).
...
LINT_SCRIPT = REPO_ROOT / "scripts" / "db" / "lint_gf_migration_scope.py"
...
def run_test(sql_file: Path) -> tuple[bool, str]:
    """Run scope lint on a test case and compare against expected output."""
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"Scope lint adversarial tests: {passed} passed, {failed} failed")
    print(f"{'='*60}")

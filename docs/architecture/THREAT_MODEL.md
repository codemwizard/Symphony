...
- Assets: `execution_records.*` (all columns of the append-only execution-truth ledger — `execution_id`, `tenant_id`, `project_id`, `interpretation_version_id`, `input_hash`, `runtime_version`, `signature_key_class`, `adapter_id`, `execution_timestamp`, `output_hash`).
...
- Attack vectors: (a) UPDATE of any `execution_records` row after initial INSERT (retroactive rewrite of execution truth); (b) DELETE of any `execution_records` row (retroactive erasure of execution truth); (c) retroactive rebinding of `interpretation_version_id` to an interpretation pack that was not active at `execution_timestamp` (temporal-binding spoof); (d) INSERT of rows with NULL determinism columns (`interpretation_version_id`, `input_hash`, `runtime_version`, `signature_key_class`, `adapter_id`) to bypass the UNIQUE determinism detection and silently duplicate execution claims.
- Mitigation: `INV-EXEC-TRUTH-001` — NOT NULL on the five determinism columns + UNIQUE `execution_records_determinism_unique` (`input_hash`, `interpretation_version_id`, `runtime_version`, `signature_key_class`, `adapter_id`) + append-only trigger `execution_records_append_only` (raises `GF056` on UPDATE/DELETE) + temporal-binding trigger `execution_records_temporal_binding` (raises `GF058` on INSERT when `interpretation_version_id` does not resolve to the pack active at `execution_timestamp`) + FK `execution_records_interpretation_version_id_fkey` → `interpretation_packs(interpretation_pack_id)`. Both triggers are hardened `SECURITY DEFINER` with pinned `search_path = pg_catalog, public` per INV-008.

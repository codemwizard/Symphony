...
- Phase-0 levy/KYC schema hooks are storage-only (`levy_rates`, `ingress_attestations.levy_applicable`, `levy_calculation_records`, `levy_remittance_periods`, `kyc_provider_registry`) with explicit runtime-read/write prohibition until Phase-2 and verifier enforcement to prevent scope creep.

# TSK-P2-W8-DB-007b PLAN - Persisted timestamp enforcement
...
Enforce timestamp integrity as a distinct authoritative boundary failure.
...
Wave 8 must distinguish cryptographic validity from authorization scope and timestamp integrity. This task adds the timestamp integrity failure domain without bundling it into generic crypto invalidity.
...
| `scripts/db/verify_tsk_p2_w8_db_007b.sql` | CREATE | Deliver or update the task-controlled artifact |
| `scripts/db/verify_tsk_p2_w8_db_007b.sh` | CREATE | Deliver or update the task-controlled artifact |
| `evidence/phase2/tsk_p2_w8_db_007b.json` | CREATE | Deliver or update the task-controlled artifact |

## Stop Conditions

- Stop if the work expands beyond `timestamp integrity`.
...
### Step 1
**What:** [ID w8_db_007b_work_01] Enforce persisted-before-signing `occurred_at` semantics so replay uses the exact authoritative timestamp rather than a regenerated value.
**Done when:** [ID w8_db_007b_work_01] PostgreSQL rejects writes whose verification path uses a regenerated or non-authoritative `occurred_at` value.

### Step 2
**What:** [ID w8_db_007b_work_02] Build a verifier that distinguishes regenerated-timestamp failures.
**Done when:** [ID w8_db_007b_work_02] The verifier distinguishes regenerated-timestamp failures at `asset_batches`.

...
- final_status: `open` (pending closure by TSK-P2-PREAUTH-003-REM-01..REM-05)
...
This casefile decomposes into exactly **five derived tasks** (REM-01..REM-05) and one sibling casefile stub (`REM-2026-04-20_execution-lifecycle`). The sibling handles execution lifecycle / retry / failure-state semantics which **must not** touch the append-only contract established here.
        ...
        └── REM-01 (migration 0131: expand — nullable determinism columns)
                └── REM-02 (migration 0132: contract — NOT NULL + UNIQUE + FK NOT NULL)
                        └── REM-03 (migration 0133: append-only trigger + temporal-binding trigger)
                                └── REM-05 (verifier + CI wiring)
                                        └── REM-04 (register INV-EXEC-TRUTH-001 — fail-closed on REM-05 evidence)
                                                └── checkpoint/EXEC-TRUTH-REM
```

Fail-closed sequencing: REM-04 only flips `INV-EXEC-TRUTH-001` status to `implemented` after REM-05 produces fresh verifier evidence carrying all integrity fields. If REM-05 evidence is missing or stale, REM-04 blocks.

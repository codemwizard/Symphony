# TSK-P3-WP-008 PLAN — Conflict-of-interest and verifier independence enforcement

Task: TSK-P3-WP-008
Owner: DB_FOUNDATION
failure_signature: PHASE3.STRICT.TSK-P3-WP-008.PROOF_FAIL
canonical_reference: docs/operations/AI_AGENT_OPERATION_MANUAL.md

---
## Regulated Surface Compliance (CRITICAL)

- Reference: `REGULATED_SURFACE_PATHS.yml`
- **MANDATORY PRE-CONDITION**: MUST NOT edit any migration or regulated file without prior approval metadata.
- Approval artifacts MUST be created BEFORE editing regulated surfaces.
- Stage A: Before editing (approvals/YYYY-MM-DD/BRANCH-<branch>.md and .approval.json)
- Stage B: After PR opening (approvals/YYYY-MM-DD/PR-<number>.md and .approval.json)
- Conformance check: `bash scripts/audit/verify_approval_metadata.sh --mode=stage-a --branch=<branch>`

## Remediation Trace Compliance (CRITICAL)

- Reference: `docs/operations/REMEDIATION_TRACE_WORKFLOW.md`
- `EXEC_LOG.md` is append-only - never delete or modify existing entries.
- Markers must be present when the file is modified - not deferred to `pre_ci.sh`.
- Mandatory `EXEC_LOG.md` markers: `failure_signature`, `origin_task_id`, `repro_command`, `verification_commands_run`, `final_status`.

---
## Database Connection Context (CRITICAL)

- **Requirement**: All database interactions in verification scripts MUST use the `DATABASE_URL` environment variable.
- **Example Export**: `export DATABASE_URL="postgresql://symphony_admin:symphony_pass@localhost:5432/symphony"`
- **Docker Context**: The container is `symphony-postgres`.

---

## Objective

Conflict-of-interest and verifier independence enforcement. This task forms a
closed proof graph from work items to acceptance criteria to execution trace.

Mission scope is intentionally narrow: establish only the deterministic
conflict-of-interest and verifier-independence enforcement substrate required by
`P3-SURF-008` and `INV-308`, anchored to persisted relationship and authority
records, without importing generic application authorization, portal workflow,
runtime/verifier exchange semantics, or `P3-SURF-012` trust-boundary
implementation.

---

## Pre-conditions

- [ ] This PLAN.md has been reviewed and approved.
- [ ] Stage A Approval artifact generated for regulated changes.

- [ ] `docs/operations/TASK_ID_NOMENCLATURE.md` reviewed for task-family and wave rules.
- [ ] `docs/PHASE3/PHASE3_CAPABILITY_BOUNDARY.md` reviewed for scope boundaries.
- [ ] `docs/PHASE3/PHASE3_INVARIANT_REGISTER.md` reviewed for invariant references.

---

## Files to Change

| File | Action | Reason |
|------|--------|--------|
| `schema/migrations/0216_p3_conflict_of_interest_enforcement.sql` | CREATE | Forward-only COI and verifier-independence enforcement substrate |
| `schema/migrations/MIGRATION_HEAD` | MODIFY | Advance canonical migration head when DB task lands |
| `schema/baseline.sql` | MODIFY | Maintain stable baseline pointer after DB closure work |
| `schema/baselines/current/0001_baseline.sql` | MODIFY | Refresh current baseline snapshot |
| `schema/baselines/current/baseline.cutoff` | MODIFY | Refresh current baseline cutoff metadata |
| `schema/baselines/current/baseline.meta.json` | MODIFY | Refresh current baseline metadata |
| `schema/baselines/2026-05-18/0001_baseline.sql` | MODIFY | Record dated baseline snapshot |
| `schema/baselines/2026-05-18/baseline.normalized.sql` | MODIFY | Record dated normalized baseline snapshot |
| `schema/baselines/2026-05-18/baseline.cutoff` | MODIFY | Record dated baseline cutoff |
| `schema/baselines/2026-05-18/baseline.meta.json` | MODIFY | Record dated baseline metadata |
| `scripts/db/verify_p3_conflict_of_interest_enforcement.sh` | CREATE | Verifier for this task |
| `evidence/phase3/tsk_p3_wp_008_conflict_of_interest_enforcement.json` | CREATE | Output artifact |
| `docs/decisions/ADR-0010-baseline-policy.md` | MODIFY | Document baseline-governance closure |
| `docs/contracts/sqlstate_map.yml` | MODIFY | Register any new SQLSTATE codes introduced by the DB task |
| `docs/tasks/PHASE3_RUNTIME_TASKS.md` | MODIFY | Register runtime task in active human index |
| `docs/PHASE3/phase3_task_registry.yml` | MODIFY | Register runtime task pack as live Phase 3 inventory |
| `tasks/TSK-P3-WP-008/meta.yml` | MODIFY | Update status upon success |
| `docs/plans/phase3/TSK-P3-WP-008/EXEC_LOG.md` | MODIFY | Append completion data |

---

## Stop Conditions

- **If approval metadata is not created before editing regulated surfaces** -> STOP
- **If EXEC_LOG.md does not contain all required markers** -> STOP
- **If the verifier fails to execute negative tests transactionally** -> STOP
- **If evidence is statically faked instead of derived** -> STOP
- **If the task expands into generic application authorization, portal workflow, or P3-SURF-012 trust-boundary implementation** -> STOP
- **If the implementation needs more than one forward-only migration for the declared COI-only objective** -> STOP
- **If the implementation requires undeclared identity-correlation semantics or runtime-only trust state** -> STOP

---

## Non-Goals

- No generic application authorization redesign.
- No product authorization or user-journey permissioning.
- No external verifier portal behavior.
- No runtime/verifier artifact exchange or privilege-separation implementation owned by `P3-SURF-012`.
- No policy or authority lineage definition, regulator partition semantics, or host-country workflow runtime.

---

## Proof Boundaries

- Guarantees:
  - The verifier can prove that deterministic COI enforcement structures exist
    and are machine-inspectable where inspected.
  - The verifier can prove that submitter-verifier overlap and related COI
    cases are structurally rejectable under the declared substrate.
- Limitations:
  - The verifier cannot prove generalized application authorization behavior,
    portal workflow behavior, or runtime/verifier exchange semantics.
  - The verifier cannot prove the substantive correctness of all role
    assignments or identity data outside the declared enforcement substrate.
  - The verifier cannot prove user-facing explanation or future-phase verifier
    workflow semantics.

---

## Implementation Steps

### Step 1: Implement Work Items
**What:** Execute the work items linked via ID.
**How:**
- [ID tsk_p3_wp_008_w01] Define a single forward-only Phase 3 schema mutation that establishes deterministic conflict-of-interest and verifier-independence enforcement structures anchored to declared submitter, verifier, asset, and authority relationships without importing generic application authorization, portal workflow, or runtime/verifier exchange semantics.
- [ID tsk_p3_wp_008_w02] Bind COI findings to immutable-lineage constitutional facts, deterministic canonical ordering and tie-break rules, replay-visible verifier-independence records, and anti-trust-collapse boundaries that never depend on runtime-only state or undeclared identity-correlation semantics.
- [ID tsk_p3_wp_008_w03] Add a deterministic verifier that proves submitter-verifier overlap and related COI cases are mechanically rejectable, and that enforcement remains anchored to persisted relationship records without absorbing P3-SURF-012 runtime/verifier segregation scope.
- [ID tsk_p3_wp_008_w04] Register the task in the active Phase 3 runtime task index and registry without altering unrelated authority-lineage, regulator partition, or future-phase verifier workflow scope.
**Done when:** All items are implemented.

### Step 2: Implement Verifier
**What:** Build the strictly mapped verifier script.
**How:**
- Implement `scripts/db/verify_p3_conflict_of_interest_enforcement.sh`.
- Connect to DB using `DATABASE_URL`.
- Enforce failure domains.
**Done when:** Script correctly evaluates acceptance criteria and exits 0 on success.

### Step 3: Emit Evidence
**What:** Run verifier and check evidence schema.
**How:**
```bash
bash scripts/db/verify_p3_conflict_of_interest_enforcement.sh > evidence/phase3/tsk_p3_wp_008_conflict_of_interest_enforcement.json
```
**Done when:** Commands exit 0 and evidence format complies.

### Step 4: Runtime Index And Registry
**What:** Register the generated runtime task pack in the active Phase 3 runtime index and registry.
**How:**
- Update `docs/tasks/PHASE3_RUNTIME_TASKS.md`.
- Update `docs/PHASE3/phase3_task_registry.yml`.
**Done when:** Both files point to `tasks/TSK-P3-WP-008/meta.yml` as the live runtime task-pack source.

### Step 5: Rebaseline (CRITICAL for DB_SCHEMA tasks)
**What:** Regenerate the physical baseline and satisfy ADR-0010 governance.
**How:**
1. Connect to DB: `export DATABASE_URL="postgresql://symphony_admin:symphony_pass@localhost:55432/symphony"`
2. Regenerate: `bash scripts/db/generate_baseline_snapshot.sh 2026-05-18`
3. Register any new SQLSTATE codes in `docs/contracts/sqlstate_map.yml`.
4. Audit Log: Append an entry to `docs/decisions/ADR-0010-baseline-policy.md` citing the new MIGRATION_HEAD and the specific changes made.
**Done when:** `scripts/db/check_baseline_drift.sh` exits 0.

---

## Verification

```bash
# 1. Task-specific verifier
bash scripts/db/verify_p3_conflict_of_interest_enforcement.sh

# 2. Migration lint
bash scripts/db/lint_migrations.sh

# 3. Evidence schema validation
python3 scripts/audit/validate_evidence.py --task TSK-P3-WP-008 --evidence evidence/phase3/tsk_p3_wp_008_conflict_of_interest_enforcement.json

# 4. Task-pack readiness
bash scripts/audit/verify_task_pack_readiness.sh --task TSK-P3-WP-008
```
